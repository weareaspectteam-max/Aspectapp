...
interface LoginProps {
  onLogin: (role: UserRole, name: string, userId: string, accessToken: string) => void;
}
...
export function Login({ onLogin }: LoginProps) {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      onLogin(role, name, user.id, session.access_token);
    } catch (err) {
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        >
          ...
          >
            ...
            <img src={logoImage} alt="Aspect Logo" className="relative w-72 h-auto drop-shadow-2xl scale-[2.8] translate-y-12" />
            <motion.div
