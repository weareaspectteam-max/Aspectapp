...
export function HamburgerMenu({ userName, userRole, onLogout, onNavigate, darkMode = false }: HamburgerMenuProps) {
  ...
  return (
    <>
      ...
      {/* Overlay + Panel — document.body'ye portal ile render edilir,
          App header'ının backdrop-blur stacking context'inden kaçar */}
      {createPortal(
        <>
          {/* Overlay */}
          <AnimatePresence>
          ...
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                className="fixed top-0 right-0 h-screen w-80 bg-gradient-to-b from-[#2a2a3a] via-[#3a3a4e] to-[#2f3439] shadow-2xl z-[9999] flex flex-col overflow-hidden border-l border-white/10"
              >
